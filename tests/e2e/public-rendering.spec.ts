...
test.describe('generated public rendering', () => {
  ...
  test('public content remains available with JavaScript disabled', async ({ browser }) => {
    ...
    for (const route of routes) {
      ...
      await expect(primaryNav.getByRole('link', { name: 'Services' })).toHaveAttribute('href', '/services')
      await expect(primaryNav.getByRole('link', { name: 'Sample audit' })).toHaveAttribute('href', '/example-audit')
      await expect(primaryNav.getByRole('link', { name: 'Methodology' })).toHaveAttribute('href', '/methodology')
      await expect(primaryNav.getByRole('link', { name: 'Plans' })).toHaveAttribute('href', '/pricing')
    }
    await context.close()
  })
