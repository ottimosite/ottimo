...
async function assertRenderedPage(page: any, path: string) {
  ...
  const onRequestFailed = (request: any) => { if (request.failure()?.errorText !== 'net::ERR_ABORTED') failedRequests.push(`${request.method()} ${request.url()} — ${request.failure()?.errorText ?? 'unknown failure'}`) }
  ...
  const axeResults = await page.evaluate(async (source) => {
    const script = document.createElement('script'); script.textContent = source; document.documentElement.appendChild(script)
...
test.describe('@rendered public pages', () => { for (const path of publicRoutes) test(`renders ${path}`, async ({ page }) => { await assertRenderedPage(page, path) }) })
test.describe('@rendered application pages', () => { for (const path of applicationRoutes) test(`renders ${path}`, async ({ page }) => { await assertRenderedPage(page, path) }) })