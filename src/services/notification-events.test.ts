...
const event = (overrides: Partial<AuditCompletedEvent> = {}): AuditCompletedEvent => ({
...
describe('notification event architecture', () => {
  it('stores and filters tenant and website events without cross-tenant leakage', () => {
    ...
    repository.append(event({
      id: 'audit.changed:audit-3:2026-09-20T14:00:00.000Z',
      type: 'audit.changed',
      sourceId: 'audit-3',
  ...
  it('rejects duplicate event identifiers', () => {
    const repository = new MemoryEventRepository()
    repository.append(event())
    expect(() => repository.append(event())).toThrow('EVENT_ID_ALREADY_EXISTS')
  })
