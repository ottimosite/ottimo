...
export function WebsiteDetail() {
  ...
  return <div className="stack website-workspace">
    ...
    {!latestAudit ? <Card className="empty-state">
      ...
      {model && <Card id="website-model" className="health-model-panel">
...
export function InsightsPage() {
  ...
  if (!audit) return <div className="stack">
    <PageHeading eyebrow="Insights" title="Understand what matters." description="Insights need an audit baseline. Ottimo will keep observed evidence separate from inference and unavailable data." />
  ...
  return <div className="stack insights-workspace">
    <PageHeading eyebrow={<>Insights{website ? ` · ${website.name}` : ''}</>} title="Turn audit evidence into useful understanding." description="One website context, one evidence boundary. These insights interpret the latest audit without pretending to know traffic, rankings, conversions or user behaviour." action={<Link className="btn btn-primary" to={`/app/audits/${audit.id}`}>Open latest audit</Link>} />
...
export function SettingsPage() { return <div className="narrow stack"><PageHeading eyebrow="Settings" title="Keep the demo aligned to your work." description="These controls stay in this browser. No account or external service is required." /><Card><h2>Workspace</h2><label>Workspace name<input defaultValue="Ottimo demo workspace" /></label><label>Default website<select defaultValue="example.com"><option>example.com</option><option>ottimo.test</option></select></label><p className="muted">Local audit data is stored in your browser and can be cleared through browser storage controls.</p><Button variant="secondary" onClick={() => window.alert('Demo settings are already saved locally.')}>Save settings</Button></Card></div> }