...
export function ResourcePerformancePanel({ audit }: { audit: Audit }) {
  ...
  return <Card className="resource-performance-panel">
    <div className="section-head"><div><span className="eyebrow">Resource intelligence</span><h2>Where page weight and delay were observed</h2><p className="section-subtitle">These are browser observations. They provide attribution evidence, not proof that a single resource caused a Core Web Vital.</p></div><span className="standard-tag">Observed only · {summary.resourceCount} resources</span></div>
    {summary.resourceCount === 0 ? <p className="muted">No browser resource data was retained for this audit.</p> : <>
      ...
      <div className="resource-performance-grid">
        <section aria-labelledby="resource-weight-heading"><h3 id="resource-weight-heading">Observed weight by type</h3><div className="profile-list">{summary.byType.slice(0, 6).map(item => <span key={item.type}><strong>{item.type}</strong> · {item.count} resources · {formatBytes(item.transferBytes)}</span>)}</div></section>
        ...
        <section aria-labelledby="resource-contributors-heading"><h3 id="resource-contributors-heading">Top observed contributors</h3><p className="section-subtitle">Ranked from measured transfer size and resource duration. This is attribution evidence, not causal proof.</p><div className="resource-list">{summary.contributors.slice(0, 8).map(item => <div key={item.pageUrl + item.url}><code title={item.url}>{shortUrl(item.url)}</code><span>{item.type} · {item.transferShare !== undefined ? Math.round(item.transferShare * 100) + '% transfer' : 'transfer unavailable'} · {item.durationShare !== undefined ? Math.round(item.durationShare * 100) + '% duration' : 'duration unavailable'}</span></div>)}</div></section>
      </div>
    </>}
  </Card>
}
