...
import { Badge, Button, Card, PageHeading, Progress } from '../../components/ui'
...
export function AuditDetail() { const { id } = useParams(); const audit = id ? localRepository.findAudit(id) : undefined; if (!audit) return <Card><h1>Audit not found</h1><Link to="/app/audits">Back to audits</Link></Card>; return <div className="stack"><PageHeading eyebrow="Audit result" title={audit.url} description={`${formatDate(audit.createdAt)} · completed in ${audit.durationMs}ms`} action={<div className="result-score"><strong>{audit.score ?? "—"}</strong><span>{audit.score === undefined ? "not measured" : "/ 100"}</span></div>} /><div className="grid-3">{audit.scores.map(score => <Card key={score.category}><span className="muted">{categoryLabels[score.category]}</span><div className="mini-score"><strong>{score.score ?? "—"}</strong>{score.score === undefined ? <small>Not measured</small> : <Progress value={score.score} />}</div></Card>)}</div><Card><div className="section-head"><div><span className="eyebrow">Findings</span><h2>What needs attention</h2></div><span className="muted">{audit.issues.length} findings</span></div><IssueTable issues={audit.issues} /></Card></div> }
