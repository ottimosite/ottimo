...
export function Recommendations() {
  ...
  const all = audits.flatMap(audit => audit.issues.map(issue => ({ ...issue, auditId: audit.id })))
  const shown = useMemo(() => [...all]
    .filter(issue => (category === 'all' || issue.category === category) && (status === 'all' || issue.status === status) && `${issue.title} ${issue.solution}`.toLowerCase().includes(query.toLowerCase()))
