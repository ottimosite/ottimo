...
export interface AuditReport { engineVersion:string; run:{id:string;status:'completed'|'failed';startedAt:string;completedAt:string;durationMs:number}; page?:PageSnapshot; error?:AuditError; measurements:AuditMeasurement[]; checks:AuditCheck[]; findings:AuditFinding[]; evidence:AuditEvidence[] }
