...
export class SiteAuditEngine {
  ...
  async audit(request: SiteAuditRequest): Promise<SiteAuditReport> {
    ...
    const first = await this.pageEngine.audit({ ...request, url: firstUrl, timeoutMs: Math.min(request.timeoutMs ?? 10_000, 15_000) })
      ...
      while (cursor < queue.length && pages.length < maxPages) {
        ...
        try {
          ...
          const report = await this.pageEngine.audit({
            ...request,
            url: safe.href,
            timeoutMs: Math.min(request.timeoutMs ?? 10_000, 15_000),
          })
